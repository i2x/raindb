...
pred_fname=paste(mon_names,"_predictors.txt",sep="")
...
# LOAD GCM DATA
gcm_fname=paste(mon_names,"_predictors.txt",sep="")
...
const=sy_rain
sy=1975			# sy is the starting year of fitting (change as needed)
ye=ye_rain-1		# ye is the ending year of fitting (change as needed)
...
best_lag=$$LAGTIME$$		# the lag time of forecast (change as needed)

mth=3			# starting month of season
mon=11			# ending month of season

# INITIAL VALUES OF FORECASTING
sfy=ye_gcm-1		# sfy is the starting year of forecasting (change as needed)
yef=ye_gcm		# yef is the ending year of forecasting (change as needed)
...
name_p=paste(pred_index[4],sep="")}
if(v == 4)
{fpreds=cbind(fpreds1[,2],fpreds1[,4])
gcm_fpreds=cbind(gcm_fpreds1[2],gcm_fpreds1[,4])
name_p=paste(pred_index[2],"_",pred_index[4],sep="")}
if(v == 5)
{fpreds=cbind(fpreds1[,4])
gcm_fpreds=cbind(gcm_fpreds1[,4])
name_p=paste(pred_index[4],sep="")}
...
gcm_fpreds=cbind(gcm_fpreds1[,3])
name_p=paste(pred_index[3],sep="")}
if(v == 11)
{fpreds=cbind(fpreds1[,3],fpreds1[,5])
gcm_fpreds=cbind(gcm_fpreds1[,3],gcm_fpreds1[,5])
name_p=paste(pred_index[3],"_",pred_index[5],sep="")}
if(v == 12)
{fpreds=cbind(fpreds1[,5])
gcm_fpreds=cbind(gcm_fpreds1[,5])
name_p=paste(pred_index[5],sep="")}
...
DF1=data.frame(year,month,X_preds,yfVol_preds)

XP1=matrix(nrow=(ty-1),ncol=cX_preds)
yfVol1=matrix(nrow=(ty-1),ncol=1)
for(e in 1:(ty-1))
{
sDF1=subset(DF1,DF1$year==yr[e])
sDF1=subset(sDF1,sDF1$month>mon)
sDF2=subset(DF1,DF1$year==yr[(e+1)])
sDF2=subset(sDF2,sDF2$month<mth)
sDF=rbind(sDF1,sDF2)
sDF=na.exclude(sDF)
ro=length(sDF$year)
if(ro != 3)
...
DF2=data.frame(yr[1:(ty-1)],XP1,yfVol1)
...
XPf1=matrix(nrow=(tyf-1),ncol=cX_preds)

for(e in 1:(tyf-1))
{
sDFf1=subset(DFf,DFf$year1==yrf[e])
sDFf1=subset(sDFf1,sDFf1$month1>mon)
sDFf2=subset(DFf,DFf$year1==yrf[(e+1)])
sDFf2=subset(sDFf2,sDFf2$month1<mth)
sDFf=rbind(sDFf1,sDFf2)
...
DFf2=data.frame(yrf[(tyf-1)],XPf1)
...
yr_pdf=sfy:(yef-1)		# years making PDF plot
nyr=length(yr_pdf)
#limit=c(381.7,528.9)	# p20th and p80th of seasonal rainfall (change as needed)
limit=c(30.3,68.9)	# p30th and p70th of seasonal rainfall (change as needed)
